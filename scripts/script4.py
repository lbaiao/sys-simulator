...
sim = system_simulation(sim_type, lower_lim_uc_db, lower_lim_d2d_db, sinr_margin, n_orthogonal_resources, n_mues, n_d2d, bs, allocation_algorithm_enum.RANDOM)

# simulation execution
n_loops = 3e3
d2d_distances_list = np.linspace(0, bs.radius/2, 20)
allocation_avg, distances_appearances = sim.run_loops(int(n_loops), d2d_distances_list)

plt.figure(1)
plt.plot(d2d_distances_list, allocation_avg)
plt.xlabel('Distância do enlace D2D [m]')
plt.ylabel('Razão de alocação')

plt.figure(2)
plt.bar([2*(i+1) for i in range(len(d2d_distances_list))], distances_appearances, align='center', tick_label= [ f'{i:.3g}' for i in d2d_distances_list] )
